export default {
  name: 'extendedwheel',
  description: 'Spin the extended prize wheel for Dredcoin.',
  aliases: [],
  usage: '<bet>',
  category: 'gambling',
  perm: 0,
  ...
  globalCooldown: 0,
  id: 72,
  dependencies: `commandEmbed formatAmount config addDredcoin getDredcoin randomNumber parseBet
  ...
  execute: async (message, args, user, command, dep) => {
    ...
    const prizes = [
      { name: 'Blue', emoji: '🟦', count: 4, multiplier: 1.1 },
      { name: 'Red', emoji: '🟥', count: 2, multiplier: 1.7 },
      { name: 'Orange', emoji: '🟧', count: 2, multiplier: 1.3 },
      { name: 'Yellow', emoji: '🟨', count: 10, multiplier: 1 },
      { name: 'Black', emoji: '⚫', count: 4, multiplier: 0 },
    ];
    const path = [
      [1, 2],
      [1, 3],
    ...
    const totalSteps = stopOffset + Math.floor((await dep.randomNumber()) * 10) + 5;
    let currentStep = 0;
    const embed = await dep.commandEmbed({
      title: `${dep.config.PREFIX}${command} ${bet}`,
      description: '**🎰 Spinning the extended wheel...**',
      color: '#FFD700',
      user,
      reward: false,
      message,
    ...
    const animate = async () => {
      if (currentStep < totalSteps) {
        ...
        const rotatedList = [...prizeList.slice(offset), ...prizeList.slice(0, offset)];
        const embed = await dep.commandEmbed({
          title: `${dep.config.PREFIX}${command} ${bet}`,
        ...
        const offset = currentStep % path.length;
        const finalList = [...prizeList.slice(offset), ...prizeList.slice(0, offset)];
